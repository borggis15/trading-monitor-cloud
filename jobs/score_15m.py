...
def action_from_thresholds(proba_up, ev, meta: dict | None) -> tuple[str, dict]:
    """
    Usa thresholds guardados en meta["thresholds"] si existen.
...
def main():
    ...
    processed = 0
    horizon_days = int(cfg["ml"]["horizon_bars"])  # en tu proyecto lo mantuviste como días
    ...
    for inst in cfg["universe"]:
        ...
        if clf is None or reg is None:
            print(f"[SCORE SKIP] {name}: no model yet")
            continue
        ...
        # acción por thresholds dinámicos (v2)
        pre_action, thr_used = action_from_thresholds(proba, ev, meta)

        # gate de robustez: si pre_action=BUY y no pasa robust => HOLD

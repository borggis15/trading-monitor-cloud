...
def read_bars(engine, exchange: str, symbol: str) -> pd.DataFrame:
    df = pd.read_sql(text("""
      ...
      order by ts
    """), engine, params={"exchange": exchange, "symbol": symbol})
    if df.empty:
        return df
    ...
    with engine.begin() as conn:
        conn.execute(text("""
          ...
          on conflict (exchange,symbol,ts) do update set
            ...
            atr=excluded.atr, zscore=excluded.zscore, ret_fwd=excluded.ret_fwd
...
def upsert_signal(engine, row: dict):
    with engine.begin() as conn:
        conn.execute(text("""
          ...
          on conflict (exchange,symbol,ts) do update set
            action=excluded.action, proba_up=excluded.proba_up, ret_exp=excluded.ret_exp, risk_est=excluded.risk_est,
            ev_bps=excluded.ev_bps, size_eur=excluded.size_eur, sl_price=excluded.sl_price, tp_price=excluded.tp_price,
            horizon=excluded.horizon, explanation=excluded.explanation, model_id=excluded.model_id
        """), row)
    ...
    with engine.begin() as conn:
        conn.execute(text("""
          ...
          on conflict (exchange,symbol) do update set
            run_at=excluded.run_at, sharpe=excluded.sharpe, sortino=excluded.sortino, max_drawdown=excluded.max_drawdown,
            win_rate=excluded.win_rate, profit_factor=excluded.profit_factor, trades=excluded.trades
...
def main():
    ...
    for inst in cfg["universe"]:
        ...
        df = provider.fetch_15m(symbol=symbol, exchange=exchange, interval=cfg["data"]["interval"], outputsize=5000)
        ...
        if not last.empty:
            ...
            risk_est = atr/price if price>0 else None

        ev = ev_bps(ret_exp, risk_est, float(cfg["backtest"]["fee_bps"]), float(cfg["backtest"]["slippage_bps"]))

        action = "HOLD"
        expl = []
        if proba is not None: expl.append(f"Prob(subida)= {proba:.2f}")
        if ret_exp is not None: expl.append(f"Ret exp ≈ {ret_exp*100:.2f}%")
        if ev is not None: expl.append(f"EV ≈ {ev:.1f} bps")
        ...
        size_eur = sl = tp = None
        if action == "BUY" and not last.empty:
            atr = float(last["atr"].iloc[0]); price = float(bars["close"].iloc[-1])
    ...
    allowed = set((r["symbol"], r["ts"]) for r in buys_sorted[:maxpos])

    for r in scored:
        if r["action"]=="BUY" and (r["symbol"], r["ts"]) not in allowed:
            r["action"] = "HOLD"
            r["size_eur"] = None
            r["sl_price"] = None
            r["tp_price"] = None
            r["explanation"] = (r["explanation"] + " | No top EV (límite posiciones)")[:2000]
        upsert_signal(engine, r)

    print("OK:", len(scored), "symbols scored")
